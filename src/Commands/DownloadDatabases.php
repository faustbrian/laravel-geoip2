...
final class DownloadDatabases extends Command
{
    ...
    public function handle(): void
    {
        ...
        foreach (Config::get('geoip2.editions') as $edition) {
            try {
                ...
                if (\str_ends_with((string) $edition, '-CSV')) {
    ...
    private function decompress(string $edition): void
    {
        ...
        $pharData = new \PharData($this->getCompressed($edition));
        $pharData->decompress();
    ...
    private function extract(string $edition): void
    {
        ...
        $pharData = new \PharData($this->getDecompressed($edition));
        $pharData->extractTo($this->getExtractPath($edition));
    ...
    private function move(string $edition): void
    {
        ...
        $finder
            ...
            ->depth(1)
            ->filter(fn (SplFileInfo $file): bool => \str_ends_with($file->getPathname(), '.'.$extension));
